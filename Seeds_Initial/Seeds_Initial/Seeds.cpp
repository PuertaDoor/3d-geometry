...
static float correl(const Image<byte>& im1, int i1,int j1,float m1,
    ...
    for(int j=-win; j<=win; j++) {
        for(int i=-win; i<=win; i++){
            float val1 = im1(i1+i,j1+j) - m1;
            float val2 = im2(i2+i,j2+j) - m2;
            dist += val1 * val2;
            norm1 += val1 * val1;
            norm2 += val2 * val2;
        }
    }
    return dist / (sqrt(norm1 * norm2) + EPS);
}
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            ...
            if(bestNCC > nccSeed) {
                disp(x,y) = dBest;
                seeds(x,y) = true;
                Q.push(Seed(x, y, dBest, bestNCC));
            } else {
                disp(x,y) = dmin-1; // Reset disparity for non-seeds
            }
        }
    }
    std::cout << std::endl;
            ...
            if(0<=x-win && x+win<im1.width() && 0<=y-win && y+win<maxy &&
               ! seeds(x,y)) {
                ...
                for(int d=s.d-1; d<=s.d+1; d++) {
                    if(dmin <= d && d <= dmax && 0 <= x+d-win && x+d+win < im2.width()) {
                        float ncc = ccorrel(im1, x, y, im2, x+d, y);
                        if(ncc > bestNCC) {
                            bestNCC = ncc;
                            dBest = d;
                ...
                if(bestNCC > nccSeed - 10) { // Relaxed threshold for propagation
                    disp(x,y) = dBest;
                    seeds(x,y) = true;
                    Q.push(Seed(x, y, dBest, bestNCC));
                }
