...
const float lambdaf = 0.35;  // Weight of regularization (smoothing) term
...
void build_graph(Graph<int,int,int>& G,
    ...
    double cWeight; // correlation weight
    int neighborNum; // neighbor of the current node
    int nodeNum; // current node

    // Creating nodes
    G.add_node(nx*ny*nd);

    auto node_number = [&](int x, int y, int d) -> int { // determine correspondant node like coordinates system
        // return x + y * nx + d * nx * ny;
        return x + y*nx + d*nx*ny;
    };

    auto rho = [&](double x) -> double { // rho function as presented in course
        if (x <= 1){
            return sqrt(1-x);
        }
        else {
    ...
    for(int x = 0; x < nx; x++) {
        for(int y = 0; y < ny; y++) {
            for(int d = 0; d < nd; d++) {
                int nodeNum = node_number(x, y, d);
                if (x > 0) { // Horizontal neighbor
                    neighborNum = node_number(x - 1, y, d);
                    G.add_edge(neighborNum, nodeNum, lambda, lambda);
                }
                if (y > 0) { // Vertical neighbor
                    neighborNum = node_number(x, y - 1, d);
                    G.add_edge(neighborNum, nodeNum, lambda, lambda);
                }
            }
        }
    ...
    // Managing operations with weights
    for(int x = 0; x < nx; x++) {
        for(int y = 0; y < ny; y++) {
            for(int d = 0; d < nd; d++) {
                int nodeNum = node_number(x, y, d);
                if (win + (x + d + dmin)*zoom >= I1.width() - win){ // we need it, otherwise win + (x + d + dmin)*zoom would be superior than width of the image
                    cWeight = wcc*rho(zncc(I1, I1M, I2, I2M, win+zoom*x, win+zoom*y, I1.width() - win - 1, win+y*zoom));
                }
                else {
                    cWeight = wcc*rho(zncc(I1, I1M, I2, I2M, win+zoom*x, win+zoom*y, win + (x + d + dmin)*zoom, win+y*zoom));
                }
                if (d == 0) { // source
                    G.add_tweights(nodeNum, cWeight, 0);
                }
                else { // connect current layer to its previous one
                    neighborNum = node_number(x, y, d-1);
                    G.add_edge(neighborNum, nodeNum, cWeight, 0);
                }
                if (d == nd-1) { // sink
                    G.add_tweights(nodeNum, 0, cWeight);
                }
            }
        }
    }
...
// Median Filter: This is a simple filter that replaces each pixel's value with the median of its neighboring pixel values.
// It's effective in reducing "salt-and-pepper" noise.
doubleImage apply_median_filter(const doubleImage& D, int filterSize = 3) {
    int nx = D.width(), ny = D.height();
    doubleImage filteredD(nx, ny);
            ...
            for (int dy = -filterSize/2; dy <= filterSize/2; dy++) {
                for (int dx = -filterSize/2; dx <= filterSize/2; dx++) {
                    ...
                    if (nx >= 0 && nx < D.width() && ny >= 0 && ny < D.height()) {
                        neighbors.push_back(D(nx, ny));
                    }
                }
            }

            std::sort(neighbors.begin(), neighbors.end());
            filteredD(x, y) = neighbors[neighbors.size() / 2];
        }
    }
...
doubleImage decode_graph(Graph<int,int,int>& G, int nx, int ny, int nd) {
    ...
    for(int y=0; y<ny; y++) {
        for(int x=0; x<nx; x++) {
            for(int d=0; d<nd; d++) {
                if(G.what_segment(x + y*nx + d*nx*ny) == Graph<int,int,int>::SOURCE) {
                    D(x,y) = d; // Assigning disparity value based on the segment
                }
            }
        }
    }

    // Optional: Apply post-processing to smooth out the disparity map
    // D = apply_median_filter(D, 10);
